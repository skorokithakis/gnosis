...
func notImplemented(command string) {
	fmt.Fprintf(os.Stderr, "gnosis: %s: not implemented\n", command)
	os.Exit(1)
}
...
func main() {
	...
	case "help":
		...
		default:
			fmt.Fprintf(os.Stderr, "gnosis: unknown help topic %q\n", os.Args[2])
			os.Exit(1)
		}
	case "write":
		store, err := storage.NewStore()
		if err != nil {
			fmt.Fprintf(os.Stderr, "gnosis: %v\n", err)
			os.Exit(1)
		}
	...
	case "show":
		if len(os.Args) < 3 {
			fmt.Fprintln(os.Stderr, "gnosis show: missing target argument")
			os.Exit(1)
		}
		store, err := storage.NewStore()
		if err != nil {
		...
		if err := commands.Show(store, os.Args[2], os.Stdout); err != nil {
			fmt.Fprintf(os.Stderr, "gnosis show: %v\n", err)
			os.Exit(1)
		}
	case "topics":
		store, err := storage.NewStore()
	...
	case "edit":
		if len(os.Args) < 3 {
			fmt.Fprintln(os.Stderr, "gnosis: edit: missing entry ID")
			os.Exit(1)
		}
		store, err := storage.NewStore()
		if err != nil {
	...
	default:
		fmt.Fprintf(os.Stderr, "gnosis: unknown command %q\n", os.Args[1])
		fmt.Fprintln(os.Stderr, "Run 'gnosis help' for a list of available commands.")
		os.Exit(1)
	}
}
